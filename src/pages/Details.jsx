...
const NegotiationSection = ({ itemId }) => {
  ...
  const apiLoadOffers = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        console.error('apiLoadOffers error', res.status, text);
        return [];
      }
      ...
      console.error('apiLoadOffers catch:', e);
      return [];
    }
  };
  ...
  const apiAddOffer = async (o) => {
    try {
      ...
      if (!res.ok) {
        ...
        console.error('apiAddOffer error', res.status, body);
        return { ok: false, status: res.status, body };
      }
      return { ok: true, status: res.status };
    } catch (e) {
      console.error('apiAddOffer catch:', e);
  ...
  const apiUpdateOffer = async (offerId, patch) => {
    try {
      ...
      if (!res.ok) {
        const body = await res.text().catch(() => '');
        console.error('apiUpdateOffer error', res.status, body);
      }
      return res.ok;
    } catch (e) {
      console.error('apiUpdateOffer catch:', e);
      return false;
    }
  };
  ...
  const apiRemoveOffer = async (offerId) => {
    try {
      ...
      if (!res.ok) {
        const body = await res.text().catch(() => '');
        console.error('apiRemoveOffer error', res.status, body);
      }
      return res.ok;
    } catch (e) {
      console.error('apiRemoveOffer catch:', e);
      return false;
    }
  };
  ...
  const addOffer = async () => {
    ...
    if (hasBackend) {
      setLoading(true);
      const result = await apiAddOffer(o);
      if (result.ok) {
        const list = await apiLoadOffers();
        setOffers(list);
        setMessage('');
        ...
        const errMsg = result.body || result.error || 'Gagal mengirim data ke server';
        alert(`Error (${result.status}): ${errMsg}`);
      }
      setLoading(false);
    } else {
      const next = [o, ...offers];
  ...
  return (
    <div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <button onClick={addOffer} disabled={loading} className="bg-indigo-600 text-white px-4 py-2 rounded disabled:opacity-50">
